...
set "PLATFORM_PORT=5174"
set "PLATFORM_URL=http://127.0.0.1:%PLATFORM_PORT%/teacher"
set "RUNTIME_DIR=%~dp0.runtime"
set "LOG_FILE=%RUNTIME_DIR%\teacher-vite.log"
...
if not errorlevel 1 goto :open_teacher

rem Start Vite in the background and keep its output for diagnosis.
start "Teacher Platform Server" /b cmd /d /c "call npm run dev -- --host 127.0.0.1 --port %PLATFORM_PORT% >"%LOG_FILE%" 2>&1"

echo Teacher Platform is starting...
...
echo Teacher Platform failed to start on port %PLATFORM_PORT%.
...
:open_teacher
start "" "%PLATFORM_URL%"
echo Teacher Platform is ready: %PLATFORM_URL%
exit /b 0
